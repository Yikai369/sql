...
```
Student information systems often ask for both a mother’s and a father’s name when a child is entered into the database. At first, this looks like a harmless administrative detail. But built into that design is a quiet assumption about what counts as a “real” family. Children raised by a single parent, orphans, or those with two mothers or two fathers often have to force the system to accept them—by entering “N/A,” repeating one name, or otherwise bending the rules just to make the form work. The problem isn’t the labels themselves; it’s the underlying rule that assumes every child must have one of each parent. That technical requirement turns a social expectation into a data constraint. Conformity is enforced not through policy or belief, but through the logic of the software itself. When someone’s life doesn’t fit that pattern, the system marks them as incomplete, effectively pressuring them to supply false information just to appear valid. A design that seems neutral, then, ends up enforcing a quiet hierarchy of recognition, where the “normal” family is modeled on traditional gendered structures, and anything else becomes an exception to be corrected. This assumption runs so deep in institutional data practices that it rarely feels like a decision at all—it feels inevitable, as if the database simply couldn’t function otherwise. Yet these technical conventions shape who can be recognized within the system and who fades into statistical background noise. The same logic underpins Pakistan’s NADRA database: an infrastructure that doesn’t just record social reality but encodes a worldview about who can exist within its order.
```
